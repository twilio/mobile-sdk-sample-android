  ...
  try{
    if (env.BRANCH_NAME == future_release || env.BRANCH_NAME == master || currentBuild.rawBuild.getCause(hudson.model.Cause$UserIdCause)){
      stage 'Prepare'
        timeout(prepare) {
          checkout scm
          sh "wget 'https://drive.google.com/uc?export=download&id=0B7npE-gj0q8JTGRweW5hdHA4R3c' -O twilio-authenticator-sample/google-services.json"
      }
...
def publishUnitTests() {
  publishHTML target: [
    ...
    keepAll: true,
    reportDir: 'twilio-authenticator-sample/build/reports/androidTests/connected',
  ...
  publishHTML target: [
    ...
    alwaysLinkToLastBuild: true,
    keepAll: true,
    reportDir: 'twilio-authenticator-sample/build/reports/tests/testDebugUnitTest',
    reportFiles: 'index.html',
